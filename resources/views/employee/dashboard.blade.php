...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white shadow rounded-lg p-5 text-center">
            ...
            <p class="text-2xl font-bold text-blue-600" id="emp-today-sales">UGX {{ number_format($todaySales, 0) }}</p>
        </div>
        <div class="bg-white shadow rounded-lg p-5 text-center">
            <h2 class="text-sm text-gray-500">{{ __("This Month's Sales") }}</h2>
            <p class="text-2xl font-bold text-green-600" id="emp-monthly-sales">UGX {{ number_format($monthlySales, 0) }}</p>
        </div>
        <div class="bg-white shadow rounded-lg p-5 text-center">
                ...
                <tbody class="text-gray-700" id="emp-recent-sales">
                    @forelse($recentSales as $sale)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-gray-600">UGX {{ number_format($sale->product->price ?? 0, 0) }}</td>
                            <td class="px-4 py-2">{{ $sale->quantity }}</td>
                            <td class="px-4 py-2 font-medium text-gray-900">UGX {{ number_format($sale->total_amount, 0) }}</td>
    ...
    function refreshEmployeeDashboard() {
        fetch('/api/employee/dashboard/stats')
            ...
            .then(function(d) {
                ...
                if (todayEl) todayEl.textContent = 'UGX ' + numberLocale(d.todaySales);
                if (monthEl) monthEl.textContent = 'UGX ' + numberLocale(d.monthlySales);
                ...
                if (tbody && d.recentSales && d.recentSales.length) {
                    tbody.innerHTML = d.recentSales.map(function(s) {
                        return '<tr class="border-b hover:bg-gray-50">' +
                            ...
                            '<td class="px-4 py-2 text-gray-600">UGX ' + numberLocale(s.total_amount / s.quantity) + '</td>' +
                            '<td class="px-4 py-2">' + s.quantity + '</td>' +
                            '<td class="px-4 py-2 font-medium text-gray-900">UGX ' + numberLocale(s.total_amount) + '</td>' +
